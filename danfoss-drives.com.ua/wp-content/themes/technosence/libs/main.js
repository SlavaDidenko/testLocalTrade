...
function setNavBtn() {
  $(".slide-nav-btn.active").removeClass("active");
  const currentNaw = $(".slide-nav-btn")[slideNow - 1];
  currentNaw.classList.add("active");
}
...
// WHY_FORM
$(".why_phone").mask("+38 (099) 999 99 99");

$(".why_form").on("submit", function (e) {
  const regEx = /[^а-яА-ЯіЇ']/;
  const regExEmail =
    /^(([^<>()[\].,;:\s@"]+(\.[^<>()[\].,;:\s@"]+)*)|(".+"))@(([^<>()[\].,;:\s@"]+\.)+[^<>()[\].,;:\s@"]{2,})$/iu;
  const regExPhone = /^\+380\d{3}\d{2}\d{2}\d{2}$/;
  e.preventDefault();

  const name = $(this).find(`.why_input[name="name"]`);
  const surname = $(this).find(`.why_input[name="surname"]`);
  const email = $(this).find(`.why_input[name="email"]`);
  const phone = $(this).find(`.why_input[name="phone"]`);

  const validName = regEx.test(name.val());
  const validSurName = regEx.test(surname.val());
  const validEmail = regExEmail.test(email.val());
  const validPhone = regExPhone.test(phone.val());

  if (validName || name.val().trim() === "") {
    name.addClass("invalid");
  }
  if (validSurName || surname.val().trim() === "") {
    surname.addClass("invalid");
  }
  if (validEmail || email.val().trim() === "") {
    email.addClass("invalid");
  }
  if (validPhone || phone.val().trim() === "") {
  ...
  sendData =
    ...
    if (sendData) {
        const comments = $(this).find(`.comments`).val()
        const userData = {
            name: name.val(),
            surname: surname.val(),
            email: email.val(),
